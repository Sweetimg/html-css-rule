...
> `images`, `js`, `less` , `views`,`fonts`五个目录是为了和现有项目保持一致，避免修改过大，所以保持不变。

> `views` 目录，用于存放前端开发做的静态页面。

> `images` 目录，用于存放静态页面开发需要的图片。

> `less` 目录，用于存放渲染页面的样式代码。

> `js` 目录，目前包括以下文件 `controllers`（控制器）,`dircetives`（指令）,`modules`（存放以模块划分的路由文件）,`filters`（过滤器）,`servies`（服务）。
...
> 那么该模块的文件名应该为：`m-detail.less`

* less页面文件，其文件名必须与HTML文件名一致；

> 假定有一个HTML页面叫 `product.html`，那么其相对应的less页面文件名应该为：`product.less`
...
那么开发人员能快速找到与该页面相关的3个直接less文件，包括：`product.less`, `m-list.less`, `m-info.less`

<a name="indentation"></a>
### 4.缩进

使用tab（2个空格宽度）来进行缩进，请自行设置
...
* 为每个less文档显示的定义编码，在文档首行定义 `@charset "utf-8";`

> 在 less 中，如果文档中出现中文，却未显示定义编码，将会编译出错，为了统一各种写法，且提前规避错误几率，统一要求每个less文档都需要定义编码
...
不允许：
	...
	<a href=http://www.vilsale.com class=home>乡村销客</a>
...
推荐：
	...
	<img src="banner.jpg" alt="每天不到一块钱，销售变化看得见。" />
...
假定有一个模块如前文 [HTML模块化](#html-module)，那么 `m-detail.less` 的写法大致如下：
...
> 任何超过4级的选择器，需要思考是否必要，是否有无歧义的，能唯一命中的更简短的写法
